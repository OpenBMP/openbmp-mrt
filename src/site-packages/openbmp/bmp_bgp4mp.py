...
class MrtBgp4MpWriter(multiprocessing.Process):
    ...
    def process_msg(self, msg):
        ...
        if logger_hash not in self.PEER_LOGGERS:
            filepath = os.path.join(self._cfg['mrt_logging']['log_dir']['bgp4mp'],
    ...
    def init_logger(self, name, filename):
        ...
        handler = RawTimedRotatingFileHandler(
                            filename=filename,
                            when=self._cfg['mrt_logging']['rotate']['when'],
                            interval=self._cfg['mrt_logging']['rotate']['interval'],
                            backupCount=self._cfg['mrt_logging']['rotate']['backupCount'],
                            delay=True)
