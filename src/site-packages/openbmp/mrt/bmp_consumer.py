...
from openbmp.mrt.mrt_bgp4mp_object import MrtBgpMpObject
from openbmp.mrt.logger import init_mp_logger
...
from openbmp.mrt.bmp import bmp_parse_bmphdr, bmp_parse_peerhdr
...
class BMPConsumer(multiprocessing.Process):
    ...
    def process_router_msg(self, msg):
        ...
        for row in map_list:
            if len(row):
                try:
                    self.LOG.info("router: [%s] %s %s", row[MsgBusFields.ACTION.getName()],
                    ...
                    if row[MsgBusFields.ACTION.getName()] in ('first', 'init'):
                        ...
                        if row[MsgBusFields.HASH.getName()] not in self.ROUTERS:
                            self.ROUTERS[row[MsgBusFields.HASH.getName()]] = {'ip': msg.getRouterIp(), 'name': row[MsgBusFields.NAME.getName()]}
                    # else:
    ...
    def process_peer_msg(self, msg):
        ...
        for row in map_list:
            if len(row):
                try:
                    peer_key = row[MsgBusFields.ROUTER_HASH.getName()] + '_' + row[MsgBusFields.REMOTE_BGP_ID.getName()] + '_' + row[MsgBusFields.PEER_RD.getName()]

                    self.LOG.info("peer: [%s] %s %s", row[MsgBusFields.ACTION.getName()],
                                  row[MsgBusFields.NAME.getName()], row[MsgBusFields.REMOTE_IP.getName()]) # Peer IP address

                    if row[MsgBusFields.ACTION.getName()] in ('up'):

                        asn_len = 2
                        if ('4 Octet ASN' in row[MsgBusFields.ADV_CAP.getName()] and
                            '4 Octet ASN' in row[MsgBusFields.RECV_CAP.getName()]):
                            asn_len = 4

                        # Update the peer hash/cache
                        if row[MsgBusFields.HASH.getName()] not in self.PEERS:
                            self.PEERS[peer_key] = {'remote_ip': row[MsgBusFields.REMOTE_IP.getName()],
                                                    'remote_asn': row[MsgBusFields.REMOTE_ASN.getName()],
                                                    'local_ip': row[MsgBusFields.LOCAL_IP.getName()],
                                                    'local_asn': row[MsgBusFields.LOCAL_ASN.getName()],
                                                    'asn_len': asn_len,
                                                    'rd': row[MsgBusFields.PEER_RD.getName()]}
                except:
    ...
    def process_bmp_raw_msg(self, msg):
        ...
        if bmp_hdrs['type'] in ('ROUTE_MON'):
            ...
            mrt_bgp_msg.BMP_DATA = data[48:]

            mrt_bgp_msg.PEER_ADDR = peer_hdr['addr']
            mrt_bgp_msg.PEER_RD = peer_hdr['dist_id']
            mrt_bgp_msg.PEER_ASN = peer_hdr['asn']
            mrt_bgp_msg.PEER_BGP_ID = peer_hdr['bgp_id']

            if long(peer_hdr['ts_secs']) == 0:
                mrt_bgp_msg.TIMESTAMP_SEC = time.time()
            else:
                mrt_bgp_msg.TIMESTAMP_SEC = peer_hdr['ts_secs']

            mrt_bgp_msg.TIMESTAMP_USEC = peer_hdr['ts_usecs']

            mrt_bgp_msg.PEER_KEY = r_hash + '_' + mrt_bgp_msg.PEER_BGP_ID + '_' + str(mrt_bgp_msg.PEER_RD)

            if c_hash in self.COLLECTORS:
                mrt_bgp_msg.COLLECTOR_ADMIN_ID = self.COLLECTORS[c_hash]['admin_id']

            if r_hash in self.ROUTERS:
                mrt_bgp_msg.ROUTER_NAME = self.ROUTERS[r_hash]['name']

            if mrt_bgp_msg.PEER_KEY in self.PEERS:
                mrt_bgp_msg.LOCAL_ADDR = self.PEERS[mrt_bgp_msg.PEER_KEY]['local_ip']
                mrt_bgp_msg.LOCAL_ASN = self.PEERS[mrt_bgp_msg.PEER_KEY]['local_asn']
                mrt_bgp_msg.PEER_ASN_LENGTH = self.PEERS[mrt_bgp_msg.PEER_KEY]['asn_len']

            # queue message
            self._bgp4mp_queue.put(mrt_bgp_msg)
